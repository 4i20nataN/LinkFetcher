...
const VIDEO_PRESETS = [
  ...
  { id: '720p', label: '720 HD', height: 720, format: 'bv*[height<=720][ext=mp4]+ba[ext=m4a]/bv*[height<=720]+ba/b[height<=720]' },
  { id: '480p', label: '480 SD', height: 480, format: 'bv*[height<=480][ext=mp4]+ba[ext=m4a]/bv*[height<=480]+ba/b[height<=480]' },
  { id: '360p', label: '360 Baixa', height: 360, format: 'bv*[height<=360][ext=mp4]+ba[ext=m4a]/bv*[height<=360]+ba/b[height<=360]' },
] as const;
...
function getMaxVideoHeight(formats: MediaInfo['formats']): number {
  ...
  for (const f of formats) {
    if (f.type === 'video' || f.type === 'image') {
      const xy = f.quality.match(/(\d+)\s*x\s*(\d+)/);
      if (xy) {
